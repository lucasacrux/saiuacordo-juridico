...
// nunca devolve null/undefined para campos string NOT NULL
const nn = (v, fallback = '') => (v ?? fallback).toString();
...
async function parseSectionGeneric(scope, headerRx, opts = {}) {
  ...
  return await handle.evaluate((root, wantDates, wantCNJ) => {
    const norm = s => (s||'').replace(/\s+/g,' ').trim();
...
async function getSectionRootStrict(scope, headerRx) {
  const handle = await scope.evaluateHandle((rxStr) => {
    const rx = new RegExp(rxStr, 'i');
    const norm = (s) => (s||'').replace(/\s+/g,' ').trim();
    const isTitle = (t) => /(movimenta|peti(?:ç|c)[oõ]es|incident|recurso|execu(?:ç|c)[aã]o|apenso|entranhad|unificad|audi(?:ê|e)ncia|sess[aã]o|partes|dados\s+do\s+processo|resumo|inform)/i
      .test(norm(t));
    ...
    // Se o título está dentro de Movimentações, aborta
    let cur = all[idx];
    while (cur && cur !== document.body) {
      if (/movimenta/i.test(norm(cur.textContent||''))) return null;
      cur = cur.parentElement;
    }

    // Define o contêiner da seção: do título até o próximo título relevante
    let parent = all[idx].parentElement || document.body;
    let startIndex = -1, endIndex = parent.children.length;
    for (let i=0;i<parent.children.length;i++) {
      if (parent.children[i].contains(all[idx])) { startIndex = i; break; }
    }
    if (startIndex === -1) return null;

    for (let j=startIndex+1;j<parent.children.length;j++) {
      const t = norm(parent.children[j].textContent||'');
      if (isTitle(t)) { endIndex = j; break; }
    }

    const wrapper = document.createElement('div');
    for (let k=startIndex;k<endIndex;k++) wrapper.appendChild(parent.children[k].cloneNode(true));
    return wrapper;
  }, headerRx.source);

  try { return await handle.asElement() ? handle : null; } catch { return null; }
}
async function extractPeticoes_v2(page) {
  return await runInAllFrames(page, async (scope) => {
    await expandInScope(scope, /peti(?:ç|c)[oõ]es(?:\s+diversas)?/i);
    const root = await getSectionRootStrict(scope, /peti(?:ç|c)[oõ]es/i);
    if (!root) return [];
    const rows = await root.evaluate((sec) => {
      ...
      for (const tr of sec.querySelectorAll('table tr')) {
        ...
        const tipo = cols.find(c=>!DATE.test(c)) || null;
        if (data || tipo) out.push({ data, tipo: tipo||joined, texto: joined });
      }
      // fallback: linhas soltas
      if (!out.length) {
        for (const el of sec.querySelectorAll('li,p,div')) {
          const t = norm(el.innerText||el.textContent||'');
          if (!t) continue; const m=t.match(DATE);
          if (m) out.push({ data: m[0], tipo: t.replace(m[0],'').replace(/^\s*[-–—:\s]+/,'').trim()||t, texto: t });
        }
      }
      return out.slice(0,400);
    });
    return rows.filter(r=>r.data || r.tipo).map(r=>({ ...r, tipo: (r.tipo||'').replace(/\bmovimenta\w*/i,'').trim() }));
  });
}

async function extractIncidentes_v2(page) {
  return await runInAllFrames(page, async (scope) => {
    await expandInScope(scope, /(incidentes?|a[cç][oõ]es?\s+incidentais|recursos?|execu(?:ç|c)[aã]o(?:es)?\s+de?\s+senten[cç]a)/i);
    const root = await getSectionRootStrict(scope, /(incidentes?|a[cç][oõ]es?\s+incidentais|recursos?|execu(?:ç|c)[aã]o(?:es)?\s+de?\s+senten[cç]a)/i);
    if (!root) return [];
    const rows = await root.evaluate((sec) => {
      const norm = (s)=> (s||'').replace(/\s+/g,' ').trim();
      const DATE = /\b\d{2}\/\d{2}\/\d{4}\b/;
      ...
      return lines.map(t=>({
        ...
        cnjs: (t.match(new RegExp(CNJ,'g'))||[]),
        type: (TYPE.test(t) ? t.match(TYPE)[0] : null)
      })).filter(x=>!(/movimenta/i.test(x.texto)));
    });
    return rows;
  });
}

async function extractApensos_v2(page) {
  return await runInAllFrames(page, async (scope) => {
    await expandInScope(scope, /(apensos?|entranhad[oa]s?|unificad[oa]s?)/i);
    const root = await getSectionRootStrict(scope, /(apensos?|entranhad[oa]s?|unificad[oa]s?)/i);
    if (!root) return [];
    const items = await root.evaluate((sec)=>{
      const norm = (s)=> (s||'').replace(/\s+/g,' ').trim();
      const DATE = /\b\d{2}\/\d{2}\/\d{4}\b/;
      const CNJ  = /\b\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}\b/;
      const out = [];
      const push = (t)=>{ if (!t) return; const mCNJ = t.match(new RegExp(CNJ,'g'))||[]; const mDT=(t.match(DATE)||[])[0]||null; out.push({ texto: norm(t), cnjs: mCNJ, apensado_em: mDT }); };
      for (const tr of sec.querySelectorAll('table tr')) {
        const t = norm(Array.from(tr.querySelectorAll('th,td')).map(td=>td.innerText||td.textContent||'').join(' '));
        push(t);
      }
      if (!out.length) {
        for (const el of sec.querySelectorAll('li,p,div')) push(norm(el.innerText||el.textContent||''));
      }
      return out;
    });
    return items.map(x=>({
      numero: x.cnjs?.[0] || null,
      classe: sanitizeClasse((x.texto.match(/classe\s*:\s*([^|]+)/i)?.[1]||'') || (x.texto.match(/\b(Execu\w+|Apela\w+|Agravo\w+|Embarg\w+|C[ií]vel|Crim\w+|Juizad\w+)/i)?.[0]||'')),
      apensado_em: x.apensado_em || null,
      motivo: (x.texto.match(/motivo\s*:\s*([^|]+)/i)?.[1]||'') || null,
      texto: x.texto,
      cnjs: x.cnjs
    })).filter(a=>a.numero || a.texto);
  });
}

async function extractAudiencias_v2(page) {
  return await runInAllFrames(page, async (scope) => {
    await expandInScope(scope, /^(?:.*\b)?(audi[eê]ncias?|sess[oõ]es?)(?:\b.*)?$/i);
    const root = await getSectionRootStrict(scope, /(audi[eê]ncias?|sess[oõ]es?)/i);
    if (!root) return [];
    const rows = await root.evaluate((sec)=>{
      const norm = (s)=> (s||'').replace(/\s+/g,' ').trim();
      const DATE = /\b\d{2}\/\d{2}\/\d{4}\b/; const TIME=/\b\d{2}:\d{2}\b/;
      const out = [];
      for (const tr of sec.querySelectorAll('table tr')) {
        const cols = Array.from(tr.querySelectorAll('th,td')).map(td=>norm(td.innerText||td.textContent||''));
        if (!cols.length) continue;
        ...
        out.push({ data, hora, tipo, descricao: joined });
      }
      if (!out.length) {
        for (const el of sec.querySelectorAll('li,p,div')) {
          const t = norm(el.innerText||el.textContent||'');
          if (!t) continue; const data=(t.match(DATE)||[])[0]||null; const hora=(t.match(TIME)||[])[0]||null;
          out.push({ data, hora, tipo: null, descricao: t });
        }
      }
      return out;
    });
    return rows.filter(a=>a.data || a.descricao);
  });
}
...
// captura HTML/estado por seção (usa busca em iframes)
async function getSectionHTML(page, headerRx) {
  if (!USE_RAW_HTML) return null;
  try {
    const h = await sliceSectionByHeader(page, headerRx);
    if (!h) return null;
    return await h.evaluate(node => node.outerHTML);
  } catch { return null; }
}

async function saveExtraSections(cnj, { peticoes, incidentes, apensos, audiencias }, sourceURL, page) {
  // limpeza idempotente
  ...
  try { await supabase.from('process_appends').delete().eq('process_cnj', cnj); } catch {}

  // status/html por seção (com iframes)
  ...
  if (Array.isArray(peticoes) && peticoes.length) {
    ...
    for (const p of peticoes) {
      const filed_at = p.data ? toISOFromBR(p.data) : null;
      const type = String(p.tipo || p.texto || '').replace(/\s+/g, ' ').trim();
      if (!type) continue;
  ...
  if (Array.isArray(incidentes) && incidentes.length) {
    ...
    for (const i of incidentes) {
      const text = String(i.texto || i.text || '').replace(/\s+/g, ' ').trim();
      if (!text) continue;
      const cnjs = i.cnjs || i.related_cnjs || [];
  ...
  // APENSOS — única tabela process_apensos
  let apsSaved = 0;
  if (Array.isArray(apensos) && apensos.length) {
    const seen = new Set();
    const rows = [];
    for (const a of apensos) {
      const numero = a.numero || (a.cnjs?.[0] || null);
      ...
      const text = nn(a.texto).trim(); // nunca null
      const related_cnjs = Array.isArray(a.cnjs) ? a.cnjs : (numero ? [numero] : []);

      const key = [numero||'', classe||'', apensado_em||'', motivo||'', text||''].join('|').toLowerCase();
      if (seen.has(key)) continue; seen.add(key);

      rows.push({
        process_cnj: cnj,
        numero, classe, apensado_em, motivo,
        text, related_cnjs, source_url: sourceURL, raw_html: stAps.raw_html
      });
    }
    if (rows.length) {
      const { error } = await supabase.from('process_apensos').upsert(rows, {
        onConflict: 'process_cnj,numero,classe,apensado_em,motivo',
        ignoreDuplicates: true
      });
  ...
  if (Array.isArray(audiencias) && audiencias.length) {
    ...
    for (const a of audiencias) {
      ...
      const description = String(a.descricao || a.texto || '').replace(/\s+/g, ' ').trim();
...
async function main() {
  ...
  // -------- Seções extras
  await expandSectionByHeader(page, /peti(?:ç|c)[oõ]es\s+diversas/i);
  await expandSectionByHeader(page, /(incidentes?|a[cç][oõ]es?\s+incidentais|recursos?|execu(?:ç|c)[aã]o(?:es)?\s+de?\s+senten[cç]a)/i);
  await expandSectionByHeader(page, /(apensos?|entranhad[oa]s?|unificad[oa]s?)/i);
  await expandSectionByHeader(page, /^(?:.*\b)?(audi[eê]ncias?|sess[oõ]es?)(?:\b.*)?$/i);

  const [peticoes, incidentes, apensos, audiencias] = await Promise.all([
    extractPeticoes_v2(page),
    extractIncidentes_v2(page),
    extractApensos_v2(page),
    extractAudiencias_v2(page),
  ]);
  await saveExtraSections(cnj, { peticoes, incidentes, apensos, audiencias }, page.url(), page);
  console.log('[DETAILS] Seções extras gravadas.', {
    peticoes: peticoes.length,
    incidentes: incidentes.length,
    apensos: apensos.length,
